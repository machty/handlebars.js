...
export function prepareNamedBlockSlot(/* openBlock, program, inverseAndProgram, close, inverted, locInfo */) {
  ...
  // the slot name for a named block slot is just its path
...
export function prepareBlock(openBlock, program, inverseAndProgram, close, inverted, locInfo) {
  ...
  if (inverseAndProgram) {
    if (decorator) {
      ...
      const inverseProgramBody = inverseAndProgram.program.body[0];
      const hasNamedBlockSlots = inverseProgramBody && inverseProgramBody.type === 'NamedBlockSlotStatement';
